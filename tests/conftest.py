...
from yolken_test_7a import YolkenTest7a, AsyncYolkenTest7a, DefaultAioHttpClient
from yolken_test_7a._utils import is_dict
...
logging.getLogger("yolken_test_7a").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[YolkenTest7a]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with YolkenTest7a(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncYolkenTest7a]:
